...
export class CoursesService {
  ...
  async createCourse(dto: CreateCourseDto) {
    ...
    return {
      ...
      isActive: true,          // derived (no column in DB)
  ...
  async findAllCourses() {
    ...
    return rows.map((c) => ({
      ...
      moduleCount: c._count.modules,   // safe: we included _count
  ...
  // === Status (UI calls PATCH /courses/:id/status) ===
  // Since there is no isActive column, return a 400 explaining how to enable it.
  ...
  async createLesson(dto: CreateLessonDto) {
    return this.prisma.lesson.create({
      data: {
        ...
        duration: dto.duration ?? null, // number or null
        order: dto.order,
