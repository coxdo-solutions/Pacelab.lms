// components/CourseModulesDialog.tsx
'use client';
...
// ---- Local types (keep loose to tolerate multiple uploader shapes)
...
interface Props {
  ...
  course: Course;
  onSuccess: () => void; // parent refetch hook
}

// ---- Helper: robust YT ID extractor from many shapes
function parseYouTubeIdFromAnything(input: unknown): string | null {
  ...
  // Ensure we also handle accidental arrays
  for (const c of candidates) {
    const id = extractId(c);
    if (id) return id;
  }

  // Some uploaders return only the 11-char id in .id or .videoId
...
export function CourseModulesDialog({ open, onOpenChange, course, onSuccess }: Props) {
  ...
  const [newLessonData, setNewLessonData] = useState<{
    moduleId: string;
    title: string;
    uploadedVideo: any | null; // accept any shape from uploader
  ...
  const addLesson = useMutation({
    mutationFn: async ({
      ...
      moduleId: string;
      title: string;
      video: any; // tolerate many return shapes
    }) => {
      const module = modules.find((m) => m.id === moduleId);
      const nextOrder =
        (module?.lessons?.map((l) => l.order).reduce((a, b) => Math.max(a, b), 0) ?? 0) + 1;

      // 🔑 Normalize YouTube ID safely from uploader result
      const vid = parseYouTubeIdFromAnything(video);
      if (!vid) {
        throw new Error('Could not parse YouTube video ID from upload result.');
      }

      // Optional duration if present (either number of seconds or ISO 8601 already parsed elsewhere)
      const dur =
      ...
      const payload = {
        moduleId,
        title,
        youtubeId: vid, // canonical 11-char ID saved to DB
